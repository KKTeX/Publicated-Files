...
As follows, \verb|\kakko| for example, is actually not fits in accurate zenkaku width: 

\begin{quotation}
  \fbox{\kakko{123}}\fbox{あ}\kakko{123}あ
\end{quotation}

However, if you want to made its width fit in accurate zenkaku width, you can use them together as follows:

\begin{lstlisting}
  \fbox{\ichimoji{\kakko{123}}}\fbox{あ}\ichimoji{\kakko{123}}あ
\end{lstlisting}
...
Personally, I don't think those should always be used together.
The reason is that, when I originally designed \verb|\kakko|, I intentionally settled on the current adjustment out of concern that forcing it to fit strictly within a zenkaku-width space would compromise its visual appearance.
This package was never intended to pursue "uniform zenkaku width alignment" as its goal; rather, the decision was made to prioritize providing versatile and visually pleasing commands.
Therefore, please regard the recommended usage as a fallback measure—something to be applied only when the layout is severely disrupted due to the lack of zenkaku width alignment.
...
\begin{lstlisting}
  ...
  \ProvidesPackage{KKsymbols}[2025/10/18, Version 1.0.4]
