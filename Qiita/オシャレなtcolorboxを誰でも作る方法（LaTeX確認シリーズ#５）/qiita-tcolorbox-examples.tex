...
\verb|borderline| を 2 つ重ねた、タイトルなしの引用カード風。

\begin{DoubleBox}
borderline を 2 行書くだけで、外側に実線、内側 1.2mm に破線、という二重枠が手に入る。
タイトルを載せていないので軽量な引用ブロックとして使える。
\end{DoubleBox}

\section*{Example C — CornerBox}
TikZ \verb|underlay| で 4 隅にブラケットだけ描いたミニマルカード。

\begin{CornerBox}{Tech Note}
4 隅の短い L 字だけで「囲ってる感」を出すデザイン。
紙資料やテックスライドでよく見る形。
\end{CornerBox}
...
水色 × 白のミニマムテックボックス。\verb|breakable| を ON にしているので、
長文を入れるとページをまたいで自動で折り返されつつ、装飾が破綻しないようになっている。
\end{TechCard}

\section*{Example D — TechCard（breakable）}
ページをまたぐと、underlay first / middle / last に分配された描画が機能する。
